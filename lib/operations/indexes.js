...
function dropIndex(mOptions) {
  const _drop = (tableName, columns, options = {}) => {
    ...
    return `DROP INDEX${concurrentlyStr}${ifExistsStr} ${mOptions.literal(
      indexName
    )}${cascadeStr};`;
  };
  return _drop;
...
function createIndex(mOptions) {
  const _create = (tableName, columns, options = {}) => {
    ...
    */
    ...
    return `CREATE ${unique} INDEX ${concurrently} ${mOptions.literal(
      indexName
    )} ON ${mOptions.literal(
      tableName
    )}${method} (${columnsString}${opclass})${where};`;
  };
