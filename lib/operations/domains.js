...
function dropDomain(mOptions) {
  const _drop = (domainName, { ifExists, cascade } = {}) => {
    ...
    return `DROP DOMAIN${ifExistsStr} ${mOptions.literal(
      domainName
    )}${cascadeStr};`;
  };
  return _drop;
...
function createDomain(mOptions, typeShorthands) {
  const _create = (domainName, type, options = {}) => {
    ...
    return `CREATE DOMAIN ${mOptions.literal(
      domainName
    )} AS ${typeStr}${constraintsStr};`;
  };
...
function renameDomain(mOptions) {
  const _rename = (domainName, newDomainName) => {
    return `ALTER DOMAIN ${mOptions.literal(
      domainName
    )} RENAME TO ${mOptions.literal(newDomainName)};`;
  };
