...
const parseReferences = (options, literal) => {
  ...
  if (referencesConstraintName) {
    clauses.push(`CONSTRAINT ${literal(referencesConstraintName)}`);
  }
  clauses.push(
...
const parseColumns = (tableName, columns, mOptions) => {
  ...
  return {
    columns: _.map(columnsWithOptions, (options, columnName) => {
      ...
      if (references) {
        constraints.push(parseReferences(options, mOptions.literal));
      }
      if (deferrable) {
...
const parseConstraints = (table, options, genName, literal) => {
  const {
    check,
    unique,
    primaryKey,
  ...
  if (check) {
    if (_.isArray(check)) {
      check.forEach((ch, i) => {
        const name = genName
          ? `CONSTRAINT ${literal(`${tableName}_chck_${i + 1}`)} `
          : '';
        constraints.push(`${name}CHECK (${ch})`);
      });
    } else {
      const name = genName ? `CONSTRAINT ${literal(`${tableName}_chck`)} ` : '';
  ...
  if (unique) {
    ...
    (isArrayOfArrays ? uniqueArray : [uniqueArray]).forEach(uniqueSet => {
      ...
      const name = genName
        ? `CONSTRAINT ${literal(`${tableName}_uniq_${cols.join('_')}`)} `
        : '';
      constraints.push(`${name}UNIQUE (${cols.map(literal).join(', ')})`);
    });
  }
  if (primaryKey) {
    const name = genName ? `CONSTRAINT ${literal(`${tableName}_pkey`)} ` : '';
  ...
  if (foreignKeys) {
    (_.isArray(foreignKeys) ? foreignKeys : [foreignKeys]).forEach(fk => {
      ...
      const name = genName
        ? `CONSTRAINT ${literal(`${tableName}_fk_${cols.join('_')}`)} `
        : '';
      const key = cols.map(literal).join(', ');
      constraints.push(
        `${name}FOREIGN KEY (${key}) ${parseReferences(fk, literal)}`
      );
    });
  }
  if (exclude) {
    const name = genName ? `CONSTRAINT ${literal(`${tableName}_excl`)} ` : '';
...
function dropTable(mOptions) {
  const _drop = (tableName, { ifExists, cascade } = {}) => {
    ...
    return `DROP TABLE${ifExistsStr} ${mOptions.literal(
      tableName
    )}${cascadeStr};`;
  };
  return _drop;
...
function createTable(mOptions) {
  const _create = (tableName, columns, options = {}) => {
    ...
    const constraintLines = parseConstraints(
      tableName,
      constraints,
      true,
      mOptions.literal
    ...
    const createTableQuery = `CREATE TABLE${temporaryStr}${ifNotExistsStr} ${mOptions.literal(
      tableName
    )} (
...
function addColumns(mOptions) {
  const _add = (tableName, columns, { ifNotExists } = {}) => {
    ...
    const alterTableQuery = `ALTER TABLE ${mOptions.literal(
      tableName
    )}\n${columnsStr};`;
...
function renameTable(mOptions) {
  const _rename = (tableName, newName) => {
    return `ALTER TABLE ${mOptions.literal(
      tableName
    )} RENAME TO ${mOptions.literal(newName)};`;
  };
...
function renameColumn(mOptions) {
  const _rename = (tableName, columnName, newName) => {
    return `ALTER TABLE ${mOptions.literal(
      tableName
    )} RENAME ${mOptions.literal(columnName)} TO ${mOptions.literal(newName)};`;
  };
...
function renameConstraint(mOptions) {
  const _rename = (tableName, constraintName, newName) => {
    return `ALTER TABLE ${mOptions.literal(
      tableName
    )} RENAME CONSTRAINT ${mOptions.literal(
      constraintName
    )} TO ${mOptions.literal(newName)};`;
  };
  ...
  const _add = (tableName, constraintName, expression) => {
    ...
    const constraintStr = formatLines(
      typeof expression === 'string'
        ? [expression]
        : parseConstraints(tableName, expression, false, mOptions.literal),
      `  ADD ${constraint}`
    );
    return `ALTER TABLE ${mOptions.literal(tableName)}\n${constraintStr};`;
  };
...
function dropConstraint(mOptions) {
  const _drop = (tableName, constraintName, { ifExists, cascade } = {}) => {
    ...
    return `ALTER TABLE ${mOptions.literal(
      tableName
    )} DROP CONSTRAINT${ifExistsStr} ${mOptions.literal(
      constraintName
    )}${cascadeStr};`;
  };
  return _drop;
