...
function dropOperator(mOptions) {
  const _drop = (operatorName, options = {}) => {
    ...
    const operatorNameStr = mOptions.literal(operatorName);
...
function createOperator(mOptions) {
  const _create = (operatorName, options = {}) => {
    ...
    if (commutator) {
      defs.push(`COMMUTATOR = OPERATOR(${mOptions.literal(commutator)})`);
    }
    if (negator) {
      defs.push(`NEGATOR = OPERATOR(${mOptions.literal(negator)})`);
    }
    if (restrict) {
    ...
    return `CREATE OPERATOR OPERATOR(${mOptions.literal(
      operatorName
    )}) (${defs.join(', ')});`;
  };
...
const operatorMap = mOptions => ({ type = '', number, name, params = [] }) => {
  ...
  if (String(type).toLowerCase() === 'function') {
    ...
    const paramsStr =
      params.length > 0 ? formatParams(params, mOptions.typeShorthands) : '';

    return `OPERATOR ${number} ${nameStr}${paramsStr}`;
  }

  if (String(type).toLowerCase() === 'operator') {
    const paramsStr = formatParams(params, mOptions.typeShorthands);
