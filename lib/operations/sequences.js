...
function dropSequence(mOptions) {
  const _drop = (sequenceName, { ifExists, cascade } = {}) => {
    ...
    return `DROP SEQUENCE${ifExistsStr} ${mOptions.literal(
      sequenceName
    )}${cascadeStr};`;
  };
  return _drop;
...
function createSequence(mOptions) {
  const _create = (sequenceName, options = {}) => {
    ...
    return `CREATE${temporaryStr} SEQUENCE${ifNotExistsStr} ${mOptions.literal(
      sequenceName
    )}
  ${clausesStr};`;
  };
...
function renameSequence(mOptions) {
  const _rename = (sequenceName, newSequenceName) => {
    return `ALTER SEQUENCE ${mOptions.literal(
      sequenceName
    )} RENAME TO ${mOptions.literal(newSequenceName)};`;
  };
