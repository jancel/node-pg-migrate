...
const createSchemalize = shouldDecamelize => {
  const transform = shouldDecamelize ? decamelize : v => v;
  return v => {
    if (typeof v === 'object') {
      const { schema, name } = v;
...
const simpleSchemalize = createSchemalize(false);

const t = (s, d) =>
  Object.keys(d || {}).reduce(
    (str, p) =>
      str.replace(new RegExp(`{${p}}`, 'g'), simpleSchemalize(d[p], v => v)), // eslint-disable-line security/detect-non-literal-regexp
    s
  );
